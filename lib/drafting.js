...
export function selectBlogKeywords(gscData, clientName, limit = 2) {
  const rows = gscData?.[clientName]?.topQueries ?? [];
  return rows
    .map((row) => {
      const k = row.k ?? row.q;
...
export function selectNearPageOneQueries(gscData, clientName, limit = 3) {
  const rows = gscData?.[clientName]?.topQueries ?? [];
  return rows
    .map((row) => {
      const k = row.k ?? row.q;
