...
// Powers the SEO ▸ Organic Conversions sub-tab. On-demand per property/date
// range. Conversions / revenue / transactions focus, with device + session
// revenue splits, daily series, and page / traffic / geo / engagement
// breakdowns. Filters every pull by account_name in JS (the `accounts` URL
// param is not reliable on the raw connector endpoint — see lib/gsc.js /
// lib/traffic-report.js). No deltas/compare period — deliberate, this report
// has never shown period-over-period change.

import { todayStr } from "./date-range";
...
export async function fetchConversionsReport(clientName, from, to) {
