...
// full reports) — a combined current+compare-period pull per connector (date
// dimensioned, bucketed client-side by real date comparison) powers the
// headline metrics and their period-over-period deltas in one round trip
// each, plus 3 GA4 breakdown pulls for recommendations. Filters every pull by
// account_name in JS, the reliable pattern (see lib/gsc.js / lib/traffic-report.js).
...
export async function fetchSummaryReport(clientName, from, to, compareFrom, compareTo) {
  ...
  // so bound by the union rather than assuming compare comes immediately
  // before "from".
  const boundFrom = compareFrom < from ? compareFrom : from;
  const boundTo = compareTo > to ? compareTo : to;

  // Current + compare period in ONE date-dimensioned pull per connector
  // (rather than two separate pulls) — halves the concurrent Windsor/GA4
  ...
  // property. Bucketed by real date comparison client-side below (not a
  // "YYYY-MM" prefix match — the two periods are arbitrary ranges, not
  // necessarily whole calendar months).
  const [gscRangeRows, ga4RangeRows, pageRows, devRows, chanRows] = await Promise.all([
    windsorGet("searchconsole", ["account_name", "date", "clicks", "impressions", "position"], boundFrom, boundTo, site).then(onlyGsc),
    windsorGet("googleanalytics4", ["account_name", "date", "sessions", "totalusers", "newusers", "conversions", "transactions", "totalrevenue", "ecommerce_purchases", "purchase_revenue"], boundFrom, boundTo, account).then(onlyGa4),
  ...
  // Sums a GSC daily-rows feed for rows whose date falls within [rFrom, rTo].
  function gscSum(rows, rFrom, rTo) {
  ...
  // Sums a GA4 daily-rows feed for rows whose date falls within [rFrom, rTo].
  function ga4Sum(rows, rFrom, rTo) {
  ...
  const visibility = gscSum(gscRangeRows, from, to);
  const a = ga4Sum(ga4RangeRows, from, to);
  ...
  // Compare-period equivalents, from the same combined pull — enough to derive deltas.
  const prevVisibility = gscSum(gscRangeRows, compareFrom, compareTo);
  const prevTraffic = ga4Sum(ga4RangeRows, compareFrom, compareTo);
