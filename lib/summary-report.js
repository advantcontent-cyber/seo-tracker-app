...
export async function fetchSummaryReport(clientName, year, month) {
  ...
  const [gscRows, ga4Rows, pageRows, devRows, chanRows, prevGscRows, prevGa4Rows] = await Promise.all([
    ...
    // Lightweight previous-month summary pulls, to power the trend indicators.
    windsorGet("searchconsole", ["account_name", "clicks", "impressions", "position"], pFrom, pTo, site).then(onlyGsc),
    windsorGet("googleanalytics4", ["account_name", "sessions", "totalusers", "newusers", "conversions", "transactions", "totalrevenue", "ecommerce_purchases", "purchase_revenue"], pFrom, pTo, account).then(onlyGa4),
  ]);
  ...
  // Previous-month equivalents, summary fields only — enough to derive deltas.
  const pg = prevGscRows[0] || {};
  const pa = prevGa4Rows[0] || {};
  const prevVisibility = { impressions: Math.round(pg.impressions ?? 0), clicks: Math.round(pg.clicks ?? 0), avgPos: round2(pg.position) };
  const prevTraffic = { sessions: Math.round(pa.sessions ?? 0), totalUsers: Math.round(pa.totalusers ?? 0), newUsers: Math.round(pa.newusers ?? 0) };
  const prevConversionsRaw = { conversions: Math.round(pa.conversions ?? 0), revenue: round2(pa.totalrevenue) };
