...
export async function fetchGscData() {
  ...
  // Four calls: site-level monthly roll-up, per-query detail, the
  ...
  const [siteRows, queryRows, pageRows, pageStatRows] = await Promise.all([
  ...
  // Per-page — plain URL roll-up (not query-scoped), used for the "top blog
  // posts" table. Kept unsliced (unlike topQueries' top-100 cap) so a busy
  // non-blog page with more impressions can never crowd a real blog post out
  // before the UI filters down to /blog/ URLs and takes its own top 10.
  const pageBuckets = {};
