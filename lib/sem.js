...
export async function fetchSemData() {
  ...
  // { spend, pending } — pending is true only when a MIXED_CURRENCY_TARGET
  // conversion couldn't happen (no rate for that currency/day), so the
  // caller can flag it rather than record a $0. Every other client
  // (NATIVE_CURRENCY_CLIENTS and the new default-passthrough case below)
  // never converts and is never pending here — see the per-platform
  // currency-mismatch check after addDaily for how a genuinely
  // un-combinable total (e.g. IC Khao Yai's USD Google + THB Meta) still
  // ends up correctly flagged.
  const convertSpend = (rawSpend, currency, date, client) => {
    ...
    // Default case — every client not in MIXED_CURRENCY_TARGET (whether or
    // not it's in NATIVE_CURRENCY_CLIENTS). No conversion, ever: the row's
    // raw amount passes through as-is, in its own real currency. Never
    // pending — a currency MISMATCH between platforms (not a failed rate
    // lookup) is what can make a client's combined spend untrustworthy now,
    // handled after addDaily below, not here.
    return { spend: round2(rawSpend), pending: false };
  };
  ...
  // Reconcile combined daily.spend/spendPending/currency for every client
  // NOT already handled above (NATIVE_CURRENCY_CLIENTS and
  // MIXED_CURRENCY_TARGET are both guaranteed one shared currency by
  // design, so they're skipped here). addDaily already summed
  // daily.google.spend + daily.meta.spend into daily.spend numerically —
  // correct only when both platforms actually bill in the same currency
  // (or only one side has real spend). When they genuinely differ (e.g. IC
  // Khao Yai: Google USD + Meta THB, confirmed live, Aug 2026), that sum is
  // meaningless — mark it spendPending (same "—" UI convention the old FX
  // failure used, just a different, deterministic root cause: mismatched
  // currencies, not a failed rate lookup) rather than show a false number.
  // Per-platform tabs (MetaTab/GoogleTab) are unaffected — they always show
  // their own platform's real currency via daily.google.currency /
  // daily.meta.currency, set in addDaily above, regardless of this.
  for (const [client, clientData] of Object.entries(result)) {
    if (NATIVE_CURRENCY_CLIENTS.has(client) || MIXED_CURRENCY_TARGET[client]) continue;
    for (const daily of Object.values(clientData.daily)) {
      const gCur = daily.google.currency;
      const mCur = daily.meta.currency;
      const mismatched = gCur && mCur && gCur !== mCur && daily.google.spend && daily.meta.spend;
      if (mismatched) {
        daily.spendPending = true;
      } else {
        // Prefer whichever platform actually contributed real (nonzero)
        // spend to the combined total — NOT just "whichever currency
        // happened to be set first". A platform can have a currency set on
        // a $0-spend row (e.g. an impression-only day with no clicks/cost),
        // and blindly preferring that would mislabel the currency of the
        // OTHER platform's real spend — caught before shipping, not live.
        daily.currency = (daily.meta.spend ? mCur : null) || (daily.google.spend ? gCur : null) || gCur || mCur || daily.currency;
      }
    }
  }
