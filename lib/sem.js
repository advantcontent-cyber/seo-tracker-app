...
function slot(result, client, date) {
  ...
  result[client].daily[date] ??= {
    ...
    // 2026). IG Profile Visits and Profile Followers are ALSO in that spec but
    // are not fetched here — live discovery found no data under any candidate
    // field name and neither action type appears in the account's raw Meta
    // actions array at all, so there's nothing real to sum yet (see
    // SsfbOverallTab in components/SeoTracker.jsx, which shows those two as
    // "no data" rather than a fabricated 0).
    meta:   { spend: 0, clicks: 0, impressions: 0, conversions: 0, reach: 0, clickBook: 0, purchases: 0, purchaseValue: 0, addToCart: 0, addToCartValue: 0, linkClicks: 0, landingPageViews: 0, spendPending: false },
  };
...
export async function fetchSemData() {
  ...
  // currency/day), so the caller can flag it rather than record a $0. Clients
  // in NATIVE_CURRENCY_CLIENTS skip conversion entirely — their report spec
  // calls for the account's own billing currency (e.g. Sora's THB), not USD.
  ...
  for (const client of ["Azerai Ke Ga Bay", "Azerai La Residence, Hue"]) {
    ...
    for (const date of Object.keys(result[client].daily)) {
      ...
      daily.google = { spend: round2(g.spend), clicks: g.clicks, impressions: g.impressions, conversions: g.conversions, allConversions: g.allConversions, allConversionsValue: 0, clickBook: g.clickBook, spendPending: googlePending };
