...
// a revenue one). See GOOGLE_ALL_CONVERSIONS_EXCLUDE_PREFIX below though —
// "broad bucket" turned out not to mean EVERY action belongs.
...
// For clients whose Click Book intentionally uses Google's broad
// all_conversions bucket (IC Khao Yai/Nomad Greenland — see the comment
// above), a subset of the actions swept into that bucket turned out to be
// Google Business Profile / location-signal conversions, not clicks on the
// client's own paid-search ads: "Local actions - *" (directions, menu
// views, website visits, orders, other engagements — all GBP listing
// interactions), "Store visits" (Google's location-based physical-visit
// inference), and "Business profile - *" (same GBP family, a smaller one
// only seen in April) — none of these are a click on the client's own
// site. Confirmed live, Aug 2026, via a temporary debug route against IC
// Khao Yai's Google Ads account: "Local actions - Other engagements" alone
// was 848/980/886 in Jan/Feb/Mar 2026 vs. 15-109 every other month. Prefix
// match (lowercased) against conversion_action_name — every other named
// action (Check Availability, Offer Click, Offer Book Now Click, Phone/
// Clicks to call, Email Link Click, Contact Form Submit, Newsletter Optin,
// Submit lead form) stays in the bucket; only listed clients are affected,
// everyone else's all_conversions is untouched.
//
// NOTE: this exclusion alone does NOT make Jun-Aug outrank Jan-Mar in the
// resulting Click Book — Jan-Mar still runs meaningfully higher, mainly
// from "Check Availability" (real, not GBP noise) plus an isolated "Phone
// Call Click" spike in March (675 vs. 20-70 every other month, likely a
// tracking anomaly worth asking the client about directly rather than
// coding around). What this DOES fix is the specific, confirmed category
// error — GBP listing/location signals miscounted as website clicks.
const GOOGLE_ALL_CONVERSIONS_EXCLUDE_PREFIX = {
  "IC Khao Yai": ["local actions", "store visits", "business profile"],
};

// Hung's Sep 2026 feedback (via the client's own Looker Studio reference
// filters, screenshots confirmed) supersedes IC Khao Yai's Click Book
// definition above with something narrower and campaign-scoped:
//   1. Every Google-sourced figure — not just Click Book — is restricted to
//      campaigns Advant actually manages. Confirmed live (temporary debug
//      route, Sep 2026) that this account also runs 6 "[AEC]"-prefixed
//      campaigns plus one legacy unprefixed "GDN-Heritage-HK-Feb-2023" one —
//      neither Advant's. Real Advant campaigns are consistently named
//      "[Advant] <market>_...", matching campaignMarket's own existing
//      `[Advant]` prefix-strip in components/SeoTracker.jsx.
//   2. Click Book (and, derived from it, Cost per Click Book) is redefined
//      from the broad all_conversions bucket to Google's plain `conversions`
//      metric (fractional/attribution-weighted — NOT all_conversions),
//      summed only across the conversion_action_category "OUTBOUND_CLICK" —
//      confirmed live to cover "Check Availability", "Offer Book Now
//      Click", and "Offer Click" (a superset of the single named action
//      this Click Book used before). GOOGLE_ALL_CONVERSIONS_EXCLUDE_PREFIX
//      above is unaffected/still applied — allConversions is still shown
//      elsewhere (e.g. the AI analyst notes' Google facts), and none of its
//      excluded GBP-listing actions fall in OUTBOUND_CLICK anyway.
// CAMPAIGN_NAME_FILTER is Google-only by design — Meta's campaign names for
// this client (e.g. "US_Conv_Clickbook_0926") don't use the "[Advant]"
// convention at all (confirmed live), and the client's own reference filter
// was scoped to the "ICKY_Google Ads" data source specifically — so Meta
// figures stay account-wide, untouched by this map.
const CAMPAIGN_NAME_FILTER = {
  "IC Khao Yai": (name) => /\[advant\]/i.test(name || ""),
};
const CLICK_BOOK_CATEGORY = {
  "IC Khao Yai": "OUTBOUND_CLICK",
};
...
function slot(result, client, date) {
  ...
  result[client].daily[date] ??= {
    ...
    google: { spend: 0, clicks: 0, impressions: 0, conversions: 0, allConversions: 0, allConversionsValue: 0, purchase: 0, purchaseValue: 0, addToCart: 0, addToCartValue: 0, clickBook: 0, spendPending: false, currency: null },
...
export async function fetchSemData() {
  ...
  const [gDaily, gCamp, mDaily, mCamp, mAdset, gConvActions, gConvActionsCamp, ga4Daily] = await Promise.all([
    ...
    // Same conversion_action_name breakdown as gConvActions above, but with
    // a campaign dimension too — needed to correct BOTH the daily total
    // (dayCombined's clickBook) AND the per-campaign figure ("Click Book by
    // Market") for GOOGLE_ALL_CONVERSIONS_EXCLUDE_PREFIX clients. Kept as
    // its own query rather than adding `campaign` to gConvActions itself,
    // so Sora/Azerai's already-confirmed Purchase/Add To Cart totals (which
    // read gConvActions) can't be affected by this fetch splitting rows
    // more finely. conversion_action_category + conversions also back IC
    // Khao Yai's redefined Click Book — see CAMPAIGN_NAME_FILTER/
    // CLICK_BOOK_CATEGORY above and buildCategoryClickBook below.
    windsorGet("google_ads", ["account_name", "date", "campaign", "conversion_action_name", "conversion_action_category", "conversions", "all_conversions", "all_conversions_value", "currency"], dateFrom, dateTo),
  ...
  // See GOOGLE_ALL_CONVERSIONS_EXCLUDE_PREFIX above — subtracts the
  // GBP/location-signal actions out of a listed client's all_conversions
  // bucket, both at the daily level (daily.google.allConversions, what
  // dayCombined's clickBook reads) and the per-campaign level (what
  // SummaryTab's "Click Book by Market" reads). A subtraction pass against
  // the totals addDaily/addCampaigns already accumulated from the account-
  // level query, NOT a rebuild from this row set — so any gap in this
  // query's own coverage can only ever under-correct (leave a little noise
  // in), never zero out data the account-level query actually had.
  const applyAllConversionsExclusions = (rows) => {
    for (const row of rows) {
      const client = clientForAccount(row.account_name, row.campaign);
      const prefixes = client && GOOGLE_ALL_CONVERSIONS_EXCLUDE_PREFIX[client];
      ...
      const { spend: value, pending } = convertSpend(row.all_conversions_value ?? 0, row.currency, date, client);
      const g = slot(result, client, date).daily[date].google;
      g.allConversions = Math.max(0, g.allConversions - count);
      if (!pending) g.allConversionsValue -= value;
      if (row.campaign) {
        const camp = slot(result, client, date).campaigns[date].find((c) => c.platform === "google" && c.name === row.campaign);
        if (camp) camp.allConversions = Math.max(0, (camp.allConversions ?? 0) - count);
      }
    }
  };

  // See CAMPAIGN_NAME_FILTER/CLICK_BOOK_CATEGORY above — IC Khao Yai's
  // redefined Click Book. Returns { [client]: { [date]: { [campaign]:
  // conversions } } }, built from the SAME campaign+category rows
  // applyAllConversionsExclusions reads, just summing (not subtracting)
  // `conversions` (fractional/attribution-weighted, NOT all_conversions)
  // wherever conversion_action_category matches AND the campaign passes
  // CAMPAIGN_NAME_FILTER. A separate map rather than folding into
  // addCampaigns/applyAllConversionsExclusions above, since those run once
  // per row without per-client category branching — this is only ever
  // non-empty for clients actually listed in CLICK_BOOK_CATEGORY.
  const buildCategoryClickBook = (rows) => {
    const out = {};
    for (const row of rows) {
      const client = clientForAccount(row.account_name, row.campaign);
      const category = client && CLICK_BOOK_CATEGORY[client];
      if (!category || row.conversion_action_category !== category) continue;
      const filterFn = CAMPAIGN_NAME_FILTER[client];
      if (!row.campaign || (filterFn && !filterFn(row.campaign))) continue;
      const date = dateOf(row);
      if (date < dateFrom || date > dateTo) continue;
      out[client] ??= {};
      out[client][date] ??= {};
      out[client][date][row.campaign] = (out[client][date][row.campaign] ?? 0) + Number(row.conversions ?? 0);
    }
    return out;
  };
  ...
  // Must run after addDaily/addCampaigns above — it subtracts from totals
  // they've already accumulated, not populate fresh fields.
  applyAllConversionsExclusions(gConvActionsCamp);
  addGa4Direct(ga4Daily);
  const categoryClickBook = buildCategoryClickBook(gConvActionsCamp);
  ...
  // IC Khao Yai — Hung's Sep 2026 campaign-filter/Click Book redefinition
  // (see CAMPAIGN_NAME_FILTER/CLICK_BOOK_CATEGORY above). Two steps:
  //  1. Drop every non-Advant Google campaign from result[client].campaigns
  //     entirely, and stamp each survivor's `clickBook` with the new
  //     category-based figure. Filtering at the SOURCE like this (rather
  //     than threading the filter through every consumer) means every
  //     reader — CampaignPerformanceTable, SummaryTab's Click Book by
  //     Market/Cost per Click Book by Market, and the daily rollup below —
  //     stays consistent automatically: the KPI totals always equal the
  //     sum of the campaigns actually shown.
  //  2. Re-derive daily.google (and the combined daily.* totals) from just
  //     those surviving campaigns, same reduce-and-overwrite pattern as the
  //     Azerai loop above (spread the existing object first — this client
  //     isn't in GOOGLE_CONVERSION_ACTION_MATCH, but the pattern still
  //     protects any other field a future addition might set on it).
  for (const [client, filterFn] of Object.entries(CAMPAIGN_NAME_FILTER)) {
    if (!result[client]) continue;
    for (const date of Object.keys(result[client].campaigns)) {
      result[client].campaigns[date] = result[client].campaigns[date]
        .filter((c) => c.platform !== "google" || filterFn(c.name))
        .map((c) => c.platform === "google"
          ? { ...c, clickBook: Math.round(categoryClickBook[client]?.[date]?.[c.name] ?? 0) }
          : c);
    }
    for (const date of Object.keys(result[client].daily)) {
      const daily = result[client].daily[date];
