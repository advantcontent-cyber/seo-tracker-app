...
export async function fetchSemData() {
  ...
  const [gDaily, gCamp, mDaily, mCamp, mAdset, gConvActions, gConvActionsCamp, ga4Daily] = await Promise.all([
    ...
    // a campaign dimension AND conversion_action_category too — needed to
    // build BOTH the daily total (dayCombined's clickBook) AND the
    // per-campaign figure ("Click Book by Market") for
    // OUTBOUND_CLICK_CATEGORY_CLIENTS. Kept as its own query rather than
    // adding `campaign`/`conversion_action_category` to gConvActions itself,
