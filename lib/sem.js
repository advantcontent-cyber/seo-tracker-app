...
// Clients whose report explicitly documents ONE shared billing currency
// across both platforms (per their reporting spec) — e.g. Sora's scorecard
// spec calls for native THB specifically. Sets the top-level daily.currency
// label. Every other client (not in MIXED_CURRENCY_TARGET either) now also
// shows native currency, per-platform rather than one declared client-wide
// currency — see the daily.google.currency/daily.meta.currency fields and
// the reconciliation pass at the end of fetchSemData, added Aug 2026 when
// live FX-to-USD conversion was removed for everyone but Azerai (below).
...
// report needs one consistent COMBINED currency — unlike every other client
// (which either shares one currency via NATIVE_CURRENCY_CLIENTS, or simply
// shows each platform's own real currency separately with no combined total
// when they don't match, see the reconciliation pass at the end of
// fetchSemData). Azerai's Meta accounts bill in VND; its Google Ads accounts
// bill mostly in USD (confirmed live, Aug 2026 — AZLRH's Google account also
// has some legacy VND-denominated rows, which just pass through unconverted
// since they already match the target) — so Google's USD rows get converted
// to VND via a live daily rate, while Meta's already-VND rows pass through
...
// figure exactly. This is the ONLY client still using the exchange-rate API
// (see EXCHANGE_KEY below) — removed everywhere else, Aug 2026, after that
// API started rate-limiting (429) and silently blanking Amount Spent/CPA/
// CPM for any client hitting it.
const MIXED_CURRENCY_TARGET = {
  "Azerai Ke Ga Bay": "VND",
  "Azerai La Residence, Hue": "VND",
};

// Fixed (NOT live) USD conversion rates — units of that currency per 1 USD,
// same convention as fetchDailyRates/dailyRates below. Used for every
// client NOT in NATIVE_CURRENCY_CLIENTS or MIXED_CURRENCY_TARGET (i.e.
// everyone except SSFB/Le Cercle/Sora and Azerai) — per the user (Aug
// 2026): show Amount Spent/CPC/CPM/CPA in USD for these clients, but via a
// static rate rather than a live one, since the live Exchange Rates API is
// what was rate-limiting (429) and blanking these same figures earlier
// today. A fixed rate has no such failure mode (always available) but is
// an approximation that drifts from the real rate over time — these are
// rough Aug 2026 figures, not pulled from a live source; revisit
// periodically rather than treating them as current.  Only THB/EUR/DKK are
// confirmed live as of Aug 2026 (IC Khao Yai's Meta leg, Nomad Greenland's
// Meta/Google legs respectively) — anything else falls back to pending
// (see convertSpend below) rather than a guessed rate.
const FIXED_USD_RATES = {
  THB: 35,   // Thai Baht
  EUR: 0.92, // Euro
  DKK: 6.9,  // Danish Krone
};

// Fallback rate for MIXED_CURRENCY_TARGET clients (Azerai) — used ONLY when
// fetchDailyRates' live lookup comes back empty for a given day (e.g. the
// exchange-rate API's quota is exhausted, confirmed happening again Sep
// 2026 — same gotcha as Aug 2026, when the live-rate approach was first
// chosen over this exact fixed multiplier). Matches the client's own
// original spec doc (×26000) — approximate and not updated live, but per
// the client (Sep 2026), showing an approximate number beats silently
// showing $0/pending revenue for a whole month. The live daily rate is
// still preferred and used first whenever fetchDailyRates has one.
const FIXED_MIXED_CURRENCY_FALLBACK = {
  VND: 26000,
};
...
// Add To Cart/Revenue are isolated to SPECIFIC named conversion_action_
// name(s) instead (see addGoogleConversionActions below) — exact match,
// lowercased, not a substring.
// Nomad Greenland is NOT listed here — its Click Book is still deliberately
// the broad multi-action bucket (a booking-intent metric, not a revenue
// one), so all_conversions/all_conversions_value stays correct and
// untouched for it. IC Khao Yai used to share that same "leave it alone"
// treatment too — see OUTBOUND_CLICK_CATEGORY_CLIENTS below for why (and
// how) that changed.
const GOOGLE_CONVERSION_ACTION_MATCH = {
  ...
  // AZLRH's account also has a second action, "azerai - GA4 (web)
  // purchase", alongside its native "Purchase" action. Originally assumed
  // (Aug 2026) to be a likely-duplicate GA4-imported tracking of the same
  // underlying purchases, so excluded to avoid double-counting — but the
  // client reported (Sep 2026) that AZLRH's Purchases figure didn't match
  // their own Looker Studio reference. Checked live against Looker's
  // "Total Purchases By Month" chart (Jan–Jun 2026: 2/6/2/11/0/8): the
  // native "Purchase" action alone only produces roughly HALF those
  // numbers (1/3/1/5/0/5) — summing BOTH actions matches Looker almost
  // exactly (2/6/2/9/0/8, only April off by 2, most likely attribution-
  // window drift between when Looker's report last refreshed and this
  // check, not a routing/logic gap). So for AZLRH specifically, both
  // actions are genuinely additive, not duplicates — include both.
  // NOT changed for AZKGB — no reference numbers were checked for it, so
  // its own "single native action" assumption stays as originally
  // confirmed until verified either way.
  "Azerai La Residence, Hue": {
    purchase: ["purchase", "azerai - ga4 (web) purchase"],
    addToCart: ["clickaddroomcheckout"],
  },
};

// Clients whose Click Book is isolated to conversion actions whose Google
// Ads "conversion action category" is Outbound Click, rather than Google's
// blanket all_conversions bucket. Per Hung's explicit instruction (Sept
// 2026, from Google Ads UI's own conversion-category filter — not a named-
// action match): "Google ads choose all conversions, but need to filter
// with conversion action category equal to Outbound Click."
//
// SUPERSEDES an earlier, narrower attempt at this same fix (Aug 2026,
// history at a408b0d/PR #47): that version excluded only a confirmed set
// of Google Business Profile / location-signal actions ("Local actions - *",
// "Store visits", "Business profile - *") by name prefix, while explicitly
// KEEPING Phone/Clicks to call, Contact Form Submit, Newsletter Optin, and
// Submit lead form in the bucket alongside Check Availability/Offer Click/
// Offer Book Now Click. Hung's category filter is stricter than that: those
// GBP actions are almost certainly not category Outbound Click either (so
// they're still excluded), but so are phone/form/newsletter conversions —
// those categorize as Phone Call Lead / Submit Lead Form / Sign-up, not
// Outbound Click, and are EXCLUDED under this definition even though the
// prior fix kept them. Per the user (Sept 2026): apply Hung's literal
// instruction over the prior fix's judgment call — confirm the resulting
// numbers with Hung/live data before treating this as the final word,
// since it wasn't verified against live Windsor data before shipping (see
// addGoogleConversionActions below for the category match itself).
const OUTBOUND_CLICK_CATEGORY_CLIENTS = new Set(["IC Khao Yai"]);

// The other half of Hung's Sept 2026 feedback, shipped separately (the
// category fix above already went live/confirmed before this landed) — per
// the client's own Looker Studio reference filters (screenshots confirmed):
// every Google-sourced figure on IC Khao Yai's Overall/Summary + Google
// tabs should count Advant-managed campaigns ONLY. Confirmed live (Sep
// 2026) that this account also runs 6 "[AEC]"-prefixed campaigns plus one
// legacy unprefixed "GDN-Heritage-HK-Feb-2023" campaign — neither Advant's.
// Real Advant campaigns are consistently named "[Advant] <market>_...",
// matching campaignMarket's own existing `[Advant]` prefix-strip in
// components/SeoTracker.jsx. Google-only by design — Meta's campaign names
// for this client (e.g. "US_Conv_Clickbook_0926") don't use this
// convention at all (confirmed live), and the client's own reference
// filter was scoped to the "ICKY_Google Ads" data source specifically, so
// Meta figures stay account-wide, untouched by this map.
const CAMPAIGN_NAME_FILTER = {
  "IC Khao Yai": (name) => /\[advant\]/i.test(name || ""),
};
...
function clientForAccount(accountName, campaignName) {
  ...
  return clientForNeedleMatch(s);
}

// Just the ACCOUNT_MATCH substring loop, with no campaign-prefix or
// SPLIT_ONLY_ACCOUNTS short-circuit — used directly by addGa4Direct's
// item_brand matching below, since SPLIT_ONLY_ACCOUNTS is about Google
// Ads account-level reliability specifically and doesn't apply to GA4's
// item_brand values. Reusing clientForAccount as-is for item_brand
// silently broke Azerai Ke Ga Bay: its lowercased item_brand ("azerai ke
// ga bay") matches SPLIT_ONLY_ACCOUNTS's own needle exactly, short-
// circuiting to null before ever reaching ACCOUNT_MATCH (found + fixed
// same session as the item_brand switch, Sep 18 2026 — Hue's item_brand
// has no comma so it didn't hit this, which is why only Ke Ga Bay went
// silently to zero).
function clientForNeedleMatch(lowercasedValue) {
  for (const [client, needle] of Object.entries(ACCOUNT_MATCH)) {
    if (lowercasedValue.includes(needle)) return client;
  }
  return null;
}
...
function slot(result, client, date) {
  ...
  result[client].daily[date] ??= {
    ...
    // currency — the real currency this platform's spend is denominated in
    // for this client (see the Aug 2026 "no live FX conversion for default
    // clients" change below) — null until a row sets it. Distinct from the
    // top-level daily.currency, which only applies to NATIVE_CURRENCY_CLIENTS
    // (one shared currency for the whole client); this is per-platform,
    // needed because clients like IC Khao Yai/Nomad Greenland bill Google
    // and Meta in genuinely different currencies (confirmed live, Aug 2026).
    // outboundClickConversions — OUTBOUND_CLICK_CATEGORY_CLIENTS' (IC Khao
    // Yai) Click Book: all_conversions summed ONLY where conversion action
    // category = Outbound Click (see that constant + applyOutboundClickCategory
    // below). allConversions above stays the true, unfiltered blanket bucket
    // for every client, including ICKY — read by the AI analyst-notes context
    // (notesFacts.google.allConversions in SummaryTab), not by Click Book.
    google: { spend: 0, clicks: 0, impressions: 0, conversions: 0, allConversions: 0, allConversionsValue: 0, purchase: 0, purchaseValue: 0, addToCart: 0, addToCartValue: 0, clickBook: 0, outboundClickConversions: 0, spendPending: false, currency: null },
    ...
    // safe to sum daily.
    meta:   { spend: 0, clicks: 0, impressions: 0, conversions: 0, reach: 0, clickBook: 0, purchases: 0, purchaseValue: 0, addToCart: 0, addToCartValue: 0, linkClicks: 0, landingPageViews: 0, igProfileVisits: 0, newFollowers: 0, spendPending: false, currency: null },
  };
...
export async function fetchSemData() {
  ...
  // Was "${YEAR}-03-01" — widened to Jan 1 per AZKGB's Aug 2026 feedback
  // ("Add Purchase data from January to the current month"). This is a
  // shared constant used by every client's SEM fetch, not AZKGB-specific;
  // Sora's/Azerai's "Total Purchases By Month" footnotes already
  // anticipated this exact question ("ask if you'd like January/February
  // included too"), confirmed with the user before widening it globally.
  ...
  const [gDaily, gCamp, mDaily, mCamp, mAdset, gConvActions, gConvActionsCamp, ga4Daily] = await Promise.all([
    ...
    // GOOGLE_CONVERSION_ACTION_MATCH above. Includes `campaign` (added Sep
    // 2026 — was account-level only) so addGoogleConversionActions can
    // route Azerai's rows by campaign PREFIX (AZKGB_/AZLRH_) instead of
    // account_name: both properties' Google Ads campaigns currently live
    // under the single raw "Azerai Ke Ga Bay" account (see
    // SPLIT_ONLY_ACCOUNTS), so account-name-only routing silently matched
    // neither client and left both Azerai properties' Purchase/Add To Cart
    // permanently at 0 — caught when AZLRH's figure didn't match the
    // client's own Looker Studio reference. Adding this dimension only
    // splits rows more finely (same total once summed back up); harmless
    // for every other client here, which already routes by account_name
    // alone.
    windsorGet("google_ads", ["account_name", "campaign", "date", "conversion_action_name", "all_conversions", "all_conversions_value", "currency"], dateFrom, dateTo),
    // Same conversion_action_name breakdown as gConvActions above, but also
    // carrying conversion_action_category — needed to build BOTH the daily
    // total (dayCombined's clickBook) AND the per-campaign figure ("Click
    // Book by Market") for OUTBOUND_CLICK_CATEGORY_CLIENTS. Kept as its own
    // query rather than folding into gConvActions (now that gConvActions
    // also carries `campaign`, the only remaining difference is this
    // category field) to keep that addition scoped and easy to reason
    // about.
    windsorGet("google_ads", ["account_name", "date", "campaign", "conversion_action_name", "conversion_action_category", "all_conversions", "all_conversions_value", "currency"], dateFrom, dateTo),
    ...
    // in slot() above). No currency field on this connector — GA4 always
    // reports in the property's set currency, which for Sora is already THB
    // (matches NATIVE_CURRENCY_CLIENTS), so no FX conversion is applied here.
    //
    // Uses item-level item_brand/item_revenue/items_purchased, NOT the
    // event-level page_location/purchase_revenue/ecommerce_purchases this
    // used before Sep 18 2026 — found + fixed after the client flagged a
    // revenue mismatch for Azerai Hue against their own GA4 Explore report
    // (breakdown by Item brand). Root cause: page_location is an event-
    // scoped dimension, so a purchase session that touched multiple pages
    // got its full purchase_revenue counted once per distinct page_location
    // row — a classic dimension/metric scope mismatch, same category of bug
    // as the Meta Reach overcount (gotcha #4). Confirmed via a debug route:
    // page_location-based Hue revenue matched the client's reference for
    // Jan/Feb/May/Jul but overcounted Mar (+5%), Apr (+1.5%), Jun (+2.6%),
    // and Aug (+51%) — the exact months/degree tracking how fragmented that
    // month's pre-purchase browsing was. item_brand/item_revenue matched the
    // reference EXACTLY for every month, both Azerai properties. Also
    // affected Ke Ga Bay identically (Aug alone was 62% overcounted) even
    // though nothing had reported it yet. See addGa4Direct below for the
    // matching logic this enables — the old hotel-ID/page_location parsing
    // is no longer needed since item_brand already gives each Azerai
    // property its own distinct, unambiguous string.
    windsorGet("googleanalytics4", ["account_name", "item_brand", "date", "item_revenue", "items_purchased"], dateFrom, dateTo),
  ]);
  ...
  // Live FX conversion is used ONLY for MIXED_CURRENCY_TARGET clients now
  // (Azerai) — per the user (Aug 2026), removed for everyone else. It was
  // originally applied broadly (any non-USD account converted to USD via
  // APILayer's Exchange Rates Data API), but that API started 429ing
  // (rate-limited/quota exceeded) and silently blanked Amount Spent/CPA/CPM
  // for any client hitting it — confirmed live, Aug 2026. For clients like
  // IC Khao Yai (Google bills USD, Meta bills THB) and Nomad Greenland
  // (Google bills DKK, Meta bills EUR — confirmed live), there's no shared
  // currency between platforms anyway, so a converted-to-USD combined total
  // was never fully honest even when the API worked. New behavior: spend is
  // never converted for these clients — each platform keeps its own real
  // currency (see the `currency` field on daily.google/daily.meta above),
  // and the combined daily.spend is only trusted (not spendPending) when
  // both platforms' currencies actually agree — see the post-addDaily pass
  // below. Azerai is scoped in here specifically because its report spec
  // needs ONE combined VND figure and the client explicitly chose live-rate
  // accuracy over the project's original fixed ×26000 multiplier.
  ...
  for (const rows of [gDaily, gCamp, mDaily, mCamp, mAdset, gConvActions]) {
    for (const row of rows) {
      ...
      if (!MIXED_CURRENCY_TARGET[clientForAccount(row.account_name, row.campaign)]) continue;
      currenciesUsed.add(row.currency);
    }
  }
  ...
  // reused for Google's conversion VALUE too, not just spend). Returns
  // { spend, pending, approx } — pending is true when a conversion
  // couldn't happen at all (no FIXED_USD_RATES entry for the default case
  // below); approx is true when it succeeded via FIXED_MIXED_CURRENCY_FALLBACK
  // rather than a live rate (see that constant's comment — the live-rate
  // API's quota runs out periodically).
  const convertSpend = (rawSpend, currency, date, client) => {
    ...
    if (mixedTarget) {
      ...
      if (currency === "USD") {
        ...
        const fallbackRate = FIXED_MIXED_CURRENCY_FALLBACK[mixedTarget];
        if (fallbackRate) return { spend: round2(rawSpend * fallbackRate), pending: false, approx: true };
    ...
    if (NATIVE_CURRENCY_CLIENTS.has(client)) return { spend: round2(rawSpend), pending: false };
    // Default case — every client not in MIXED_CURRENCY_TARGET or
    // NATIVE_CURRENCY_CLIENTS (i.e. not SSFB/Le Cercle/Sora, and not
    // Azerai). Converts to USD via FIXED_USD_RATES above — a static rate,
    // not a live one (see that constant's comment for why). Pending (not a
    // guessed rate) for any currency without a fixed rate on file.
    if (!currency || currency === "USD") return { spend: round2(rawSpend), pending: false };
    const rate = FIXED_USD_RATES[currency];
    if (rate) return { spend: round2(rawSpend / rate), pending: false };
    return { spend: 0, pending: true };
  };
  ...
  const addDaily = (rows, platform, withConv) => {
    for (const row of rows) {
      ...
      // Per-platform real currency (see the field's comment in slot() above)
      // — every client now needs this, not just NATIVE_CURRENCY_CLIENTS,
      // since spend is never converted for non-MIXED_CURRENCY_TARGET clients
      // anymore.
      if (row.currency) s[platform].currency = row.currency;
  ...
  const addCampaigns = (rows, platform, withConv) => {
    for (const row of rows) {
      ...
      slot(result, client, date).campaigns[date].push({
        ...
        // Real currency this row's spend is denominated in — needed so
        // callers combining campaigns across platforms (e.g. SummaryTab's
        // Cost per Click Book by Market) can tell when a market bucket
        // mixes currencies and its spend total isn't safe to show as one
        // number, same reasoning as daily.google.currency/daily.meta.currency
        // in slot() above.
  ...
  // Passes row.campaign through to clientForAccount (fixed Sep 2026 — used
  // to omit it) so Azerai's rows route by campaign prefix instead of the
  // shared/ambiguous account_name; see the comment on the gConvActions
  // fetch above for the full story.
  ...
  // See OUTBOUND_CLICK_CATEGORY_CLIENTS above — builds a listed client's
  // Click Book from ONLY conversion actions whose category is Outbound
  // Click, both at the daily level (daily.google.outboundClickConversions,
  // what dayCombined's clickBook now reads for these clients) and the
  // per-campaign level (what SummaryTab's "Click Book by Market" reads).
  // An additive build into its OWN field, unlike the exclusion-by-
  // subtraction approach this replaced — daily.google.allConversions/
  // camp.allConversions are left as the true, unfiltered blanket bucket
  // (still read by the AI analyst-notes context), not touched here.
  const applyOutboundClickCategory = (rows) => {
    for (const row of rows) {
      const client = clientForAccount(row.account_name, row.campaign);
      if (!client || !OUTBOUND_CLICK_CATEGORY_CLIENTS.has(client)) continue;
      // Normalized so this matches Windsor whether it returns the raw
      // Google Ads API enum ("OUTBOUND_CLICK") or a humanized label
      // ("Outbound Click") — see OUTBOUND_CLICK_CATEGORY_CLIENTS' comment.
      const category = (row.conversion_action_category || "").toLowerCase().replace(/_/g, " ").trim();
      if (category !== "outbound click") continue;
      const date = dateOf(row);
      if (date < dateFrom || date > dateTo) continue;
      const count = Math.round(row.all_conversions ?? 0);
      const g = slot(result, client, date).daily[date].google;
      g.outboundClickConversions += count;
      if (row.campaign) {
        const camp = slot(result, client, date).campaigns[date].find((c) => c.platform === "google" && c.name === row.campaign);
        if (camp) camp.outboundClickConversions = (camp.outboundClickConversions ?? 0) + count;
      }
    }
  };

  // See directRevenue/directPurchases comment in slot() above — the site's
  // own GA4 ecommerce numbers. Matched via item_brand (not account_name)
  // since Sep 18 2026 — see the windsorGet call above for why. item_brand
  // conveniently already carries each property's own distinct name (e.g.
  // "Sora Resort & Suites Sukhumvit", "Azerai Ke Ga Bay", "Azerai La
  // Residence Hue"), and matching it against the plain ACCOUNT_MATCH
  // needle list (clientForNeedleMatch, NOT clientForAccount — see that
  // function's comment for why) correctly disambiguates both Azerai
  // properties on its own: "ke ga bay" is checked before the broader
  // "azerai" needle (ACCOUNT_MATCH's declaration order), so "Azerai Ke Ga
  // Bay" matches its own needle first rather than falling through to
  // Hue's. No shared-property hotel-ID/page_location hack needed anymore
  // now that item_brand disambiguates directly.
  const addGa4Direct = (rows) => {
    for (const row of rows) {
      const client = clientForNeedleMatch((row.item_brand || "").toLowerCase());
      if (!client) continue;
      const date = dateOf(row);
      if (date < dateFrom || date > dateTo) continue;
      const daily = slot(result, client, date).daily[date];
      daily.directRevenue += Number(row.item_revenue ?? 0);
      daily.directPurchases += Math.round(row.items_purchased ?? 0);
    }
  };
  ...
  // Must run after addDaily/addCampaigns above — it finds and mutates the
  // campaign objects they already pushed.
  applyOutboundClickCategory(gConvActionsCamp);
  ...
  // IC Khao Yai — Hung's Sept 2026 campaign-filter feedback (see
  // CAMPAIGN_NAME_FILTER above), the other half of the Outbound Click
  // category fix above. Two steps:
  //  1. Drop every non-Advant Google campaign from result[client].campaigns
  //     entirely. Filtering at the SOURCE like this (rather than threading
  //     the filter through every consumer) means every reader —
  //     CampaignPerformanceTable, SummaryTab's Click Book by Market/Cost
  //     per Click Book by Market (via campaignsInRange), and the daily
  //     rollup below — stays consistent automatically: the KPI totals
  //     always equal the sum of the campaigns actually shown.
  //  2. Re-derive daily.google (and the combined daily.* totals) from just
  //     those surviving campaigns — same reduce-and-overwrite pattern as
  //     the Azerai loop above, now also carrying outboundClickConversions
  //     through the reduce so applyOutboundClickCategory's per-campaign
  //     work above survives this overwrite instead of being wiped.
  for (const [client, filterFn] of Object.entries(CAMPAIGN_NAME_FILTER)) {
    if (!result[client]) continue;
    for (const date of Object.keys(result[client].campaigns)) {
      result[client].campaigns[date] = result[client].campaigns[date].filter((c) => c.platform !== "google" || filterFn(c.name));
    }
    for (const date of Object.keys(result[client].daily)) {
      const daily = result[client].daily[date];
      const googleCampaigns = (result[client].campaigns[date] || []).filter((c) => c.platform === "google");
      const g = googleCampaigns.reduce((a, c) => ({
          ...
          allConversions: a.allConversions + (c.allConversions ?? 0), clickBook: a.clickBook + (c.clickBook ?? 0),
          allConversionsValue: a.allConversionsValue + (c.allConversionsValue ?? 0),
          outboundClickConversions: a.outboundClickConversions + (c.outboundClickConversions ?? 0),
        }), { spend: 0, clicks: 0, impressions: 0, conversions: 0, allConversions: 0, clickBook: 0, allConversionsValue: 0, outboundClickConversions: 0 });
      const googlePending = googleCampaigns.some((c) => c.spend == null);
      daily.google = { ...daily.google, spend: round2(g.spend), clicks: g.clicks, impressions: g.impressions, conversions: g.conversions, allConversions: g.allConversions, allConversionsValue: round2(g.allConversionsValue), clickBook: g.clickBook, outboundClickConversions: g.outboundClickConversions, spendPending: googlePending };
      daily.spend = round2(daily.google.spend + daily.meta.spend);
      daily.spendPending = googlePending || daily.meta.spendPending;
      daily.clicks = daily.google.clicks + daily.meta.clicks;
      daily.impressions = daily.google.impressions + daily.meta.impressions;
  ...
  // Reconcile combined daily.currency for every client NOT already handled
  // above (NATIVE_CURRENCY_CLIENTS and MIXED_CURRENCY_TARGET are both
  // guaranteed one shared currency by design, so they're skipped here).
  // As of the Aug 2026 fixed-rate change, default clients' daily.google.spend
  // and daily.meta.spend are BOTH already converted to USD by convertSpend
  // (or left as pending — see s.spendPending, set per-row in addDaily above,
  // which already covers the "no fixed rate for this currency" case) — so
  // daily.spend = google.spend + meta.spend is always a safe USD sum now,
  // regardless of what the two platforms' RAW currencies were. There's no
  // more "mismatched currency" case to detect here (that was the old
  // no-conversion behavior); daily.currency is just "USD" per slot()'s
  // default and needs no adjustment. Per-platform tabs (MetaTab/GoogleTab)
  // are unaffected — they always show their own platform's real currency via
  // daily.google.currency/daily.meta.currency (the RAW, pre-conversion
  // currency), set in addDaily above.
...
// Also carries `clicks` + `spend` per country — added Aug 2026 for Azerai's
// Meta tab (Impressions/Clicks by Country + Amount Spent by Country charts),
// same connector/dimension, just two more plain-count/plain-sum fields.
export async function fetchMetaCountryBreakdown(dateFrom, dateTo) {
  if (!WINDSOR_KEY) throw new Error("WINDSOR_API_KEY not set");
  const rows = await windsorGet("facebook", ["account_name", "country", "impressions", "clicks", "spend", "actions_offsite_conversion_fb_pixel_purchase"], dateFrom, dateTo);
  const result = {};
  for (const row of rows) {
    const client = clientForAccount(row.account_name);
    if (!client || !row.country) continue;
    result[client] ??= {};
    result[client][row.country] ??= { impressions: 0, purchases: 0, clicks: 0, spend: 0 };
    ...
    result[client][row.country].clicks += Math.round(row.clicks ?? 0);
    result[client][row.country].spend += row.spend ?? 0;
  }
  return result;
}
