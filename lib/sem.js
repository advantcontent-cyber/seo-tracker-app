...
// Both Azerai properties share ONE GA4 property ("azerai - GA4") — there's
// no per-property account_name split at all here (unlike Google Ads, which
// at least has campaign-name prefixes to route by). The only thing that
// distinguishes them in the GA4 data is the numeric hotel ID embedded in
// page_location's path (both properties book through the same reservations
// engine, e.g. https://reservations.azerai.com/110349?... or .../110430?...,
// occasionally via a second hostname, reservations.travelclickchina.cn,
// same ID scheme). IDs confirmed live (Sep 2026) by visiting each
// reservations.azerai.com/<id> URL directly. Checked against every GA4 row
// for the shared property over Jan-Sep 2026: 100% matched one of these two
// IDs, zero unattributable — see addGa4Direct below for how this is used.
const GA4_HOTEL_ID_MATCH = {
  "110349": "Azerai La Residence, Hue",
  "110430": "Azerai Ke Ga Bay",
};
const GA4_SHARED_PROPERTY_ACCOUNTS = new Set(["azerai - ga4"]);
...
export async function fetchSemData() {
  ...
  const [gDaily, gCamp, mDaily, mCamp, mAdset, gConvActions, gConvActionsCamp, ga4Daily] = await Promise.all([
    ...
    // so no FX conversion is applied here. `page_location` added Sep 2026 —
    // see GA4_HOTEL_ID_MATCH above, needed to split Azerai's shared property.
    windsorGet("googleanalytics4", ["account_name", "page_location", "date", "purchase_revenue", "ecommerce_purchases"], dateFrom, dateTo),
  ]);
  ...
  // — harmless, since only Sora's/Azerai's Summary tabs read these fields.
  //
  // Azerai is the one exception: both properties share a single GA4
  // property ("azerai - GA4"), so account_name alone can't tell them apart
  // (clientForAccount's plain substring match used to attribute ALL of it
  // to "Azerai La Residence, Hue" purely because that client's needle,
  // "azerai", happens to match the shared property's name too — AZKGB's
  // Direct Revenue was silently 0 as a result). Route these rows by the
  // numeric hotel ID embedded in page_location instead — see
  // GA4_HOTEL_ID_MATCH above.
  const addGa4Direct = (rows) => {
    for (const row of rows) {
      const accountKey = (row.account_name || "").toLowerCase();
      let client;
      if (GA4_SHARED_PROPERTY_ACCOUNTS.has(accountKey)) {
        const m = /^https?:\/\/[^/]+\/(\d+)/.exec(row.page_location || "");
        client = m && GA4_HOTEL_ID_MATCH[m[1]];
        if (!client) continue; // unattributable row (none found live, Sep 2026) — skip rather than guess
      } else {
        client = clientForAccount(row.account_name);
      }
      if (!client) continue;
      const date = dateOf(row);
      if (date < dateFrom || date > dateTo) continue;
