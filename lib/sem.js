...
function slot(result, client, mo) {
  ...
  result[client].monthly[mo] ??= {
    ...
    meta:   { spend: 0, clicks: 0, impressions: 0, conversions: 0, reach: 0 },
  };
...
export async function fetchSemData() {
  ...
  const [gMonthly, gCamp, mMonthly, mCamp] = await Promise.all([
    ...
    windsorGet("facebook",   ["account_name", "year_month", "clicks", "impressions", "spend", "reach", "currency"], dateFrom, dateTo),
    windsorGet("facebook",   ["account_name", "year_month", "campaign", "clicks", "impressions", "spend", "currency"], dateFrom, dateTo),
  ]);

  // Some clients' ad accounts don't bill in USD — Azerai's Meta accounts
  // (both properties) and its new La Residence Hue Google Ads account run in
  // VND, and Nomad Greenland runs Meta in EUR and Google Ads in DKK. Every
  // other account in this dashboard is USD, and there's no currency
  // conversion wired up yet (needs Windsor's exchange_rates datasource
  // connected first, then a live-rate conversion here). Rather than show
  // wildly wrong dollar figures in the meantime, non-USD rows for these
  // clients are dropped — their spend/clicks/etc. from those rows just don't
  // count yet (their SEM tab reads as "no data" until conversion is live).
  const PENDING_CURRENCY_CONVERSION = ["Azerai Ke Ga Bay", "Azerai La Residence, Hue", "Nomad Greenland"];
  const isUnconvertedNonUsd = (client, currency) =>
    PENDING_CURRENCY_CONVERSION.includes(client) && currency && currency !== "USD";
  ...
  const addMonthly = (rows, platform, withConv) => {
    for (const row of rows) {
      ...
      if (platform === "meta") s[platform].reach += Math.round(row.reach ?? 0);
