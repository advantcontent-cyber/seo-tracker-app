...
const NATIVE_CURRENCY_CLIENTS = new Set(["Sora Sukhumvit"]);
...
function slot(result, client, date) {
  ...
  result[client].daily[date] ??= {
    ...
    meta:   { spend: 0, clicks: 0, impressions: 0, conversions: 0, reach: 0, clickBook: 0, purchases: 0, purchaseValue: 0, addToCart: 0, addToCartValue: 0, spendPending: false },
  };
...
export async function fetchSemData() {
  ...
  const [gDaily, gCamp, mDaily, mCamp] = await Promise.all([
    ...
    windsorGet("facebook",   ["account_name", "date", "clicks", "impressions", "spend", "reach", "actions_offsite_conversion_fb_pixel_search", "actions_offsite_conversion_fb_pixel_purchase", "action_values_offsite_conversion_fb_pixel_purchase", "actions_offsite_conversion_fb_pixel_add_to_cart", "action_values_offsite_conversion_fb_pixel_add_to_cart", "currency"], dateFrom, dateTo),
