...
export async function fetchSemData() {
  ...
  // Residence Hue"), so the existing clientForAccount/ACCOUNT_MATCH
  // substring matching works unchanged and correctly disambiguates both
  // Azerai properties on its own — "ke ga bay" is checked before the
  // broader "azerai" needle (see ACCOUNT_MATCH's declaration order), so
  // "Azerai Ke Ga Bay" matches its own needle first rather than falling
  // through to Hue's. No shared-property hotel-ID/page_location hack
  // needed anymore now that item_brand disambiguates directly.
  const addGa4Direct = (rows) => {
    for (const row of rows) {
      const client = clientForAccount(row.item_brand);
      if (!client) continue;
      const date = dateOf(row);
      if (date < dateFrom || date > dateTo) continue;
      const daily = slot(result, client, date).daily[date];
