...
export async function fetchSemData() {
  ...
  const [gDaily, gCamp, mDaily, mCamp, mAdset, gConvActions] = await Promise.all([
...
// Meta ad creatives (thumbnail + name + performance) — Sora's Meta tab spec.
// `thumbnail_url` (confirmed live against Sora's account, Aug 2026 — real
// Facebook CDN URLs returned for all 8 active ads) is Windsor's per-ad
// creative thumbnail field. NOTE: these are Facebook's own SIGNED CDN URLs
// (expire after a window — the `oe=` param is the expiry timestamp), not
// permanent links — safe to render directly in an <img> since the dashboard
// re-fetches this on every date-range change and via the API route's
// force-dynamic + no caching, but a stale/reloaded page after the signature
// expires will 403 on the image specifically (not the whole route) — the UI
// should have an onError fallback rather than assume the URL is forever-valid.
// On-demand fetch (not part of fetchSemData's broad daily pull) for the same
// reason country breakdowns are: this dimension is only meaningful to one
// client's tab, and ad-level rows would multiply that pull's size for
// everyone else.
