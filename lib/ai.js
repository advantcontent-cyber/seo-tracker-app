...
export async function fetchAiData() {
  ...
  const [rows, pageRows] = await Promise.all([
    windsorGet(["account_name", "year_month", "session_source_medium", "sessions", "conversions"], `${YEAR}-03-01`, `${YEAR}-07-31`, accounts),
    windsorGet(["account_name", "session_source_medium", "landing_page", "sessions", "conversions"], `${YEAR}-03-01`, `${YEAR}-07-31`, accounts),
  ]);
