...
// producing bucketed data once "today" moved past July — see the matching
// MONTHS/REPORT_END_MONTH fix in components/SeoTracker.jsx and lib/gsc.js,
// which this mirrors.
...
//   pages:   [{ page, sessions, conversions, engines:[{key,label,sessions}] }], // top AI landing pages (accumulated, Mar…Jul)
//   byMonth: { [mo]: { engines, search, totals:{sessions,conversions}, bing:{..}|null, pages } }, // same shapes, scoped to one month (engines/search still carry the full `series` for trend sparklines)
//   months:  MONTHS,
// }
...
export async function fetchAiData() {
  ...
  // (both keyed by year_month so the "Top pages" table can also be scoped to
  // a single selected month, not just accumulated over the window).
  const [rows, pageRows] = await Promise.all([
    ...
    windsorGet(["account_name", "year_month", "session_source_medium", "landing_page", "sessions", "conversions"], `${YEAR}-03-01`, dateTo, accounts),
  ]);
...
export function buildAiData(rows, pageRows = []) {
  // acc[client][engineKey] = { sessions, conversions, byMonth: { 3: {sessions,conversions}, .. } }
  const acc = {};
  for (const row of rows) {
    ...
    e.sessions    += s;
    e.conversions += c;
    e.byMonth[mo] ??= { sessions: 0, conversions: 0 };
    e.byMonth[mo].sessions    += s;
    e.byMonth[mo].conversions += c;
  }

  // pacc[client][mo][page] = { sessions, conversions, byEngine: { chatgpt: sessions } }
  // Chat engines only (Bing excluded, as with the totals); "(not set)" pages
  // dropped since they aren't an actionable landing URL. Bucketed by month so
  // the "Top pages" table can be scoped to a single selected month.
  const pacc = {};
  for (const row of pageRows) {
    ...
    pacc[client]           ??= {};
    pacc[client][mo]       ??= {};
    pacc[client][mo][page] ??= { sessions: 0, conversions: 0, byEngine: {} };
    const p = pacc[client][mo][page];
    p.sessions    += s;
    p.conversions += c;
    p.byEngine[key] = (p.byEngine[key] ?? 0) + s;
  }

  // Top landing pages across the given months (a single month, or the whole
  // window), each with its per-engine session split.
  const pagesForMonths = (client, months) => {
    const combined = {};
    for (const mo of months) {
      for (const [page, p] of Object.entries(pacc[client]?.[mo] || {})) {
        combined[page] ??= { sessions: 0, conversions: 0, byEngine: {} };
        combined[page].sessions    += p.sessions;
        combined[page].conversions += p.conversions;
        for (const [k, v] of Object.entries(p.byEngine)) {
          combined[page].byEngine[k] = (combined[page].byEngine[k] ?? 0) + v;
        }
      }
    }
  ...
  for (const [client, byKey] of Object.entries(acc)) {
    const all = ENGINES
      ...
      .map((def) => {
        ...
        return {
          ...
          series: MONTHS.map((m) => e.byMonth[m]?.sessions ?? 0),
        };
      })
      .sort((a, b) => b.sessions - a.sessions);
    ...
    // Per-month breakdown — same shape as the accumulated fields below, but
    // scoped to a single month, for the tab's month selector. `series` on each
    // engine still spans the full window (kept for the trend sparkline).
    const byMonth = {};
    for (const mo of MONTHS) {
      const monthAll = ENGINES
        .filter((def) => byKey[def.key])
        .map((def) => {
          const e = byKey[def.key];
          const bm = e.byMonth[mo] || { sessions: 0, conversions: 0 };
          return {
            key: def.key, label: def.label, search: !!def.search,
            sessions: bm.sessions, conversions: bm.conversions,
            series: MONTHS.map((m) => e.byMonth[m]?.sessions ?? 0),
          };
        })
        .sort((a, b) => b.sessions - a.sessions);

      const mEngines = monthAll.filter((e) => !e.search);
      const mSearch  = monthAll.filter((e) => e.search);

      byMonth[mo] = {
        engines: mEngines,
        search:  mSearch,
        totals: {
          sessions:    mEngines.reduce((a, e) => a + e.sessions, 0),
          conversions: mEngines.reduce((a, e) => a + e.conversions, 0),
        },
        bing: mSearch.length ? {
          sessions:    mSearch.reduce((a, e) => a + e.sessions, 0),
          conversions: mSearch.reduce((a, e) => a + e.conversions, 0),
        } : null,
        pages: pagesForMonths(client, [mo]),
      };
    }
    ...
    data[client] = {
      ...
      pages: pagesForMonths(client, MONTHS),
      byMonth,
      months: MONTHS,
    };
  }
