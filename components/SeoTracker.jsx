...
function SoraSummaryTab({ client, selectedRange, compareRange, range, semData }) {
  ...
  // Revenue from Ads vs Total Direct Revenue (GA4), grouped by month — same
  // chart as AzeraiSummaryTab's, requested for Sora too Sep 2026.
  // directRevenue is GA4's own item-level item_revenue, matched by
  // item_brand (see lib/sem.js's addGa4Direct).
  ...
  return (
    <div>
      ...
      {/* Revenue from Ads vs Total Direct Revenue (GA4) — grouped monthly
          bars, same chart as AzeraiSummaryTab's, added for Sora Sep 2026. */}
...
function AzeraiSummaryTab({ client, selectedRange, compareRange, range, semData }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: C.faint, fontSize: 11.5 }} className="mt-4">
        Combined Google Ads + Meta (via Windsor), {selectedRange ? `${fmtDayLong(selectedRange.from)} – ${fmtDayLong(selectedRange.to)}` : ""}. Figures shown in VND — Meta bills natively in VND; Google Ads bills in USD and is converted to VND via live daily FX rates, falling back to the client's original spec doc's fixed ×26000 multiplier on days the live-rate API's quota is exhausted (confirmed happening again Sep 2026 — this silently zeroed Google's share of Revenue for several months until caught). Purchase/Add To Cart/Revenue read from this account's specific "Purchase"/"ClickAddRoomCheckout" conversion actions{client.name === "Azerai La Residence, Hue" ? " (plus a second \"azerai - GA4 (web) purchase\" action for Purchase/Revenue specifically — genuinely additive for this property, not a duplicate, confirmed against the client's own Looker Studio reference, Sep 2026)" : ""} — not Google's broader "All conversions" bucket, which also sums in Begin Checkout, hotline calls, etc. and would overcount both figures (fixed Aug 2026, caught by the client). Per-platform breakdowns live under the Meta and Google tabs. Total Purchases By Month always shows the full available history (from January 2026 onward) regardless of the date-range picker.
      </p>
    </div>
  );
}
...
function AzeraiGoogleTab({ client, selectedRange, compareRange, range, semData, googleSearchTerms }) {
  ...
  return (
    <div>
      ...
      <p style={{ color: C.faint, fontSize: 11.5 }} className="mt-4">
        Google Ads (via Windsor), {selectedRange ? `${fmtDayLong(selectedRange.from)} – ${fmtDayLong(selectedRange.to)}` : ""}. Figures shown in VND, converted from this account's native USD billing via live daily FX rates, falling back to a fixed ×26000 rate on days the live-rate API's quota is exhausted (confirmed happening again Sep 2026 — same gotcha as Aug 2026){cur?.spendPending ? " — pending FX conversion for part of this range" : ""}. Website Purchases/Revenue/ROAS read from this account's native "Purchase" conversion action{isAzlrh ? " and its second \"azerai - GA4 (web) purchase\" action — for this property specifically, checked live (Sep 2026) against the client's own Looker Studio reference, both are genuinely additive, not a duplicate as first assumed (Aug 2026)" : ""} — not Google's broader "All conversions" bucket, which also sums in Begin Checkout, ClickAddRoomCheckout, hotline calls, etc. (fixed Aug 2026, caught by the client). Reach and Frequency are dropped from this tab — Google Ads has no Reach metric via Windsor, unlike the doc's spec (which lists both, likely carried over from the Meta tab), so neither is computable here.{isAzlrh ? " Top Performing Keywords shows the account's real triggered search terms (Google Ads' search term report) rather than configured keyword targets — Windsor's keyword-level report can't be combined with any performance metric, confirmed live, Aug 2026 — ranked by clicks." : ""}
      </p>
    </div>
  );
}
